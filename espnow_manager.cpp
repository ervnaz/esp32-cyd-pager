...
static void _x25519(uint8_t pub[32], const uint8_t priv[32]) {
  ...
  auto fd=[](fe o,const fe a,const fe b){for(int i=0;i<10;i++)o[i]=a[i]-b[i];};
  ...
  for(int pos=254;pos>=0;pos--){
    ...
    fa(A,x2,z2);fm(AA,A,A);fd(B,x2,z2);fm(BB,B,B);
    fd(E,AA,BB);fa(C,x3,z3);fd(D,x3,z3);
    fm(DA,D,A);fm(CB,C,B);
    fa(x3,DA,CB);fm(x3,x3,x3);fd(z3,DA,CB);fm(z3,z3,z3);fm(z3,z3,x1);
...
// ── Module state — lives in exactly one translation unit ──────────────────────
static PeerInfo  _peers[MAX_PEERS];
static int       _peer_n   = 0;
static uint32_t  _msg_ctr  = 0;
static uint32_t  _last_hb  = 0;
static uint8_t   _priv[X25519_LEN];
static uint8_t   _pub[X25519_LEN];
static char      _name[DEVICE_NAME_LEN];
...
static void _upsert(const uint8_t* mac, const char* name,
...
static void _onRecv(const uint8_t* mac, const uint8_t* data, int len) {
  ...
  switch ((PacketType)data[0]) {
    case PKT_HEARTBEAT: {
      ...
      _upsert(hb->mac, hb->name, hb->pubkey);
      break;
    }
    case PKT_MESSAGE: {
      ...
      strncpy(ack.name, _name, DEVICE_NAME_LEN - 1);
...
void begin(void (*rx_cb)(const PagerMessage&)) {
  ...
  // Load or generate device name
  {
    ...
    if (saved.length() > 0 && saved.length() < DEVICE_NAME_LEN) {
      strncpy(_name, saved.c_str(), DEVICE_NAME_LEN - 1);
    } else {
      uint8_t mac[6]; esp_wifi_get_mac(WIFI_IF_STA, mac);
      snprintf(_name, DEVICE_NAME_LEN, "ND-%02X%02X%02X", mac[3], mac[4], mac[5]);
  ...
  // Load or generate X25519 keypair
  {
    ...
    bool loaded = (len == X25519_LEN &&
                   p.getBytes("priv", _priv, X25519_LEN) == X25519_LEN);
    if (!loaded) {
      esp_fill_random(_priv, X25519_LEN);
      _priv[0] &= 248; _priv[31] &= 127; _priv[31] |= 64;
      p.putBytes("priv", _priv, X25519_LEN);
      Serial.println("[CRYPTO] New X25519 key generated");
    } else {
      Serial.println("[CRYPTO] X25519 key loaded");
    }
    p.end();
    _x25519(_pub, _priv);
  }
...
bool sendMessage(const char* text) {
  ...
  strncpy(pkt.name, _name, DEVICE_NAME_LEN - 1);
  pkt.msg_id = ++_msg_ctr;
  ...
  if (r == ESP_OK && _rx_cb) {
    ...
    strncpy(sent.sender_name, _name, DEVICE_NAME_LEN - 1);
...
void update() {
  ...
  if (now - _last_hb > HEARTBEAT_MS) {
    ...
    strncpy(hb.name, _name, DEVICE_NAME_LEN - 1);
    memcpy(hb.pubkey, _pub, X25519_LEN);
...
int peerCount()    { return _peer_n; }
PeerInfo* getPeer(int i) { return (i < _peer_n) ? &_peers[i] : nullptr; }
int activePeerCount() {
  int n = 0;
